...
const createFavoriteCar = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(user) || !mongoose.Types.ObjectId.isValid(car)) {
    ...
    res.status(201).json({ message: 'Favorite car added successfully', favoriteCar });
  } catch (err) {
    if (err.code === 11000) { // Duplicate key error
      return res.status(400).json({ message: 'You already have this car on your favorites.' });
    }
...
const getFavoriteCars = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Favorite cars retrieved successfully', favoriteCars });
  } catch (err) {
    res.status(500).json({ message: 'Server error', error: err.message });
  }
};
